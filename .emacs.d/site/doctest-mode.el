...
(defvar doctest-option-alist
  '(("DONT_ACCEPT_TRUE_FOR_1" t)
    ("DONT_ACCEPT_BLANKLINE" t)
    ("NORMALIZE_WHITESPACE" t)
    ("ELLIPSIS" t)
    ("SKIP" t)
    ("IGNORE_EXCEPTION_DETAIL" t)
    ("COMPARISON_FLAGS" t)
    ("REPORT_UDIFF" t)
    ("REPORT_CDIFF" t)
    ("REPORT_NDIFF" t)
    ("REPORT_ONLY_FIRST_FAILURE" t)
    ("REPORTING_FLAGS" t)))

(defvar doctest-last-option nil)
(defvar doctest-option-history nil)
(defun doctest-append-option ()
  ;; TODO: handle existing options
  (interactive)
  (setq doctest-last-option
        (completing-read
         (if (> (length doctest-last-option) 0)
             (format "Option (default %s): " doctest-last-option)
           "Option: ")
         doctest-option-alist nil nil nil
         'doctest-option-history doctest-last-option))
  (end-of-line)
  (insert (format " #doctest: +%s" doctest-last-option)))
...
(defconst doctest-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map "\C-c\C-o" 'doctest-append-option)
